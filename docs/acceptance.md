...
| **慢轨复核（双轨）** | 真机：点 ✨ 后 `data-slow="running"`，**同刻草稿已是快轨结果** → 快轨未被阻塞；复核无实质发现时回到 `"none"`，草稿全程未被改、无残留 `running`。五条安全阀与取消路径由客户端脚本化慢轨逐条验过（自动替换 / 编辑即作废 / 失败静默 / 超窗口 offer），截图见 `docs/evidence/client-slow-*.png` | ✅ |
| **慢轨触发条件（受控计数）** | 清空网络日志后单跑两条路径，数 POST 到本插件路由的次数：**无歧义草稿 = 1 次**（仅快轨，`data-slow="none"`）；**闸门问过的草稿 = 3 次**（闸门 + 带答案快轨 + 慢轨，`data-slow` 由 `running` 转 `none`）。console 无任何 `composer-enhance` 警告，说明客户端那条"重试兜底"未被触发 | ✅ |
| **慢轨的实际产出（负面数据，它决定了触发条件）** | 真机 4 次慢轨复核，`issues` 计数为 **0 / 0 / 2 / 0**——对无歧义草稿复核多数时候"没话说"，却每次 ✨ 都要多付一次 `effort:max` 调用。**已据决策改为：只有本轮闸门确实问过（走过 chips 或 panel）才跑慢轨**；窗口同时从 8s 放宽到 30s（`effort:max` 实测延迟 1.2s–9.5s，8s 会把自动替换整条分支关掉） | ✅ 已收口 |
